...
import { apartmentFormats, complexAdvantages, complexFacts } from '@/data/sochiPark';
import { QuizModal } from './QuizModal';

const heroPhrases = ['Бизнес-класс в микрорайоне Бытха', 'До моря около 1 500 м', 'Школа и детский сад рядом', 'Планировки для жизни и аренды'];
...
function ApartmentCard({ image, alt, title, description, price, onOpen }: { image: string; alt: string; title: string; description: string; price: string; onOpen: () => void }) {
  return <article className="apartment-card"><div className="apartment-image"><Image src={image} alt={alt} width={548} height={548} /></div><div className="apartment-copy"><p className="apartment-price">{price}</p><h3>{title}</h3><small>{description}</small><p className="availability-note">Наличие, этаж и условия покупки уточним в подборке.</p><BlueButton onClick={onOpen} /></div></article>;
}
  ...
  return <>
    <main>
      <section className="hero" id="header">
        ...
        <div className="hero-inner"><h1>Квартиры в ЖК<br />«Сочи Парк»</h1><p className="hero-rotator" aria-label={heroPhrases[phraseIndex]}><span className="hero-typed" aria-hidden="true">{typedPhrase}</span><span className="hero-cursor" aria-hidden="true">|</span></p><div className="hero-finance"><div><span>ИПОТЕКА ОТ</span><strong>20 000 ₽/мес.</strong></div><div><span>ПЕРВЫЙ ВЗНОС ОТ</span><strong>0 ₽</strong></div></div><div className="purpose-card" aria-label="Выберите цель покупки"><div className="purpose-grid"><button type="button" onClick={() => openQuiz('Жить на море', 'Первый экран / Для жизни')}>Жить</button><button type="button" onClick={() => openQuiz('Отдыхать', 'Первый экран / Для отдыха')}>Отдыхать</button><button type="button" onClick={() => openQuiz('Перепродать', 'Первый экран / Перепродать')}>Перепродать</button><button type="button" onClick={() => openQuiz('Сдавать в аренду', 'Первый экран / Сдавать')}>Сдавать</button></div><p><img src="/icons/clock.svg" alt="" /><strong>1 минута</strong></p></div><div className="trust-row" aria-label="Рейтинги и партнёры"><span><Image src="/logos/logo-yandex.webp" alt="Яндекс" width={56} height={56} />4,9</span><span><Image src="/logos/logo-2gis.webp" alt="2ГИС" width={56} height={56} />4,7</span><span>Платиновый<br />партнёр</span><Image src="/logos/logo-sber.webp" alt="Сбер" width={56} height={56} /><Image src="/logos/logo-alfa.webp" alt="Альфа-Банк" width={56} height={56} /><Image src="/logos/logo-sovcom.webp" alt="Совкомбанк" width={56} height={56} /></div></div>
      </section>

      <section className="complex-section" id="complex"><div className="content"><p className="eyebrow">ЖК «СОЧИ ПАРК»</p><h2>Бизнес-класс в микрорайоне Бытха</h2><p className="complex-lead">Жилой комплекс для тех, кто выбирает Сочи для жизни, отдыха или инвестиции. Подберём формат по бюджету, этажу, виду и сценарию покупки.</p><dl className="complex-facts">{complexFacts.map((fact) => <div key={fact.label}><dt>{fact.label}</dt><dd>{fact.value}</dd></div>)}</dl></div></section>

      <section className="prices-section" id="pricesAndAssortment"><div className="content"><h2>Подборки квартир</h2><p className="section-intro">Выберите формат — менеджер пришлёт реальные доступные варианты с планировками и условиями покупки.</p><div className="apartment-grid">{apartmentFormats.map(({ trigger, ...apartment }) => <ApartmentCard key={apartment.title} {...apartment} onOpen={() => openQuiz(undefined, trigger)} />)}</div></div></section>
      ...
      <section className="advantages-section" id="advantages"><div className="content"><p className="eyebrow">О КОМПЛЕКСЕ</p><h2>Преимущества ЖК «Сочи Парк»</h2><div className="advantages-grid">{complexAdvantages.map((advantage) => <article key={advantage.title} className="advantage-card"><Image src={advantage.image} alt={advantage.alt} width={900} height={600} /><h3>{advantage.title}</h3><p>{advantage.description}</p></article>)}</div><div className="advantages-cta"><BlueButton onClick={() => openQuiz(undefined, 'Преимущества / Подборка квартир')}>ПОЛУЧИТЬ ПОДБОРКУ КВАРТИР</BlueButton></div></div></section>
